...
import { ForgotPasswordFormComponent, ForgotPasswordFormValue, AUTH_CONSTANTS } from '@/components/auth';
    ...
    <div class="px-8 min-h-screen flex justify-center items-center">
      <div class="border border-surface-200 dark:border-surface-700 bg-surface-0 dark:bg-surface-900 rounded py-16 px-6 md:px-16 z-10 w-full max-w-md">
        <app-forgot-password-form 
          [isLoading]="isLoading"
          (submitForm)="onForgotPassword($event)"
        />
      </div>
    </div>
...
export class ForgotPasswordPage {
  ...
  onForgotPassword(formValue: ForgotPasswordFormValue): void {
    this.isLoading = true;
    ...
    this.authService.forgotPassword({ email: formValue.email }).subscribe({
      next: () => {
        ...
        this.notificationService.success(
          ...
          AUTH_CONSTANTS.SUCCESS_MESSAGES.PASSWORD_RESET_EMAIL
        );
      ...
      error: (error) => {
        ...
        this.notificationService.error(
          AUTH_CONSTANTS.NOTIFICATION_TITLES.PASSWORD_ERROR,
          error?.error?.message || AUTH_CONSTANTS.ERROR_MESSAGES.EMAIL_NOT_FOUND
        );
      }
    });
  }
