...
import { RegisterFormComponent, RegisterFormValue, AUTH_CONSTANTS } from '@/components/auth';
...
@Component({
  ...
  template: `
    ...
    <div class="px-8 min-h-screen flex justify-center items-center">
      <div class="border border-surface-200 dark:border-surface-700 bg-surface-0 dark:bg-surface-900 rounded py-16 px-6 md:px-16 z-10 w-full max-w-2xl">
        <app-register-form 
          [isLoading]="isLoading"
...
export class RegisterPage {
  ...
  onRegister(formValue: RegisterFormValue): void {
    this.isLoading = true;
    ...
    this.authService.register({ firstName, lastName, email, password }).subscribe({
      next: () => {
        ...
        this.notificationService.success(
          ...
          AUTH_CONSTANTS.SUCCESS_MESSAGES.REGISTER
        );
      ...
      error: (error) => {
        ...
        this.notificationService.error(
          AUTH_CONSTANTS.NOTIFICATION_TITLES.REGISTER_ERROR,
          error?.error?.message || AUTH_CONSTANTS.ERROR_MESSAGES.REGISTER_FAILED
        );
      }
    });
  }
