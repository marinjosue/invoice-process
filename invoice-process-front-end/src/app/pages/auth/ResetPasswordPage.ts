...
import { ResetPasswordFormComponent, ResetPasswordFormValue, AUTH_CONSTANTS } from '@/components/auth';
    ...
    <div class="px-8 min-h-screen flex justify-center items-center">
      <div class="border border-surface-200 dark:border-surface-700 bg-surface-0 dark:bg-surface-900 rounded py-16 px-6 md:px-16 z-10 w-full max-w-md">
        <app-reset-password-form 
          [isLoading]="isLoading"
          (submitForm)="onResetPassword($event)"
        />
      </div>
    </div>
...
export class ResetPasswordPage {
  ...
  onResetPassword(formValue: ResetPasswordFormValue): void {
    if (!this.token) {
    ...
    this.authService.resetPassword(this.token, {
      ...
      next: () => {
        ...
        this.notificationService.success(
          ...
          AUTH_CONSTANTS.SUCCESS_MESSAGES.PASSWORD_CHANGED
        );
      ...
      error: (error) => {
        ...
        this.notificationService.error(
          AUTH_CONSTANTS.NOTIFICATION_TITLES.PASSWORD_ERROR,
          error?.error?.message || AUTH_CONSTANTS.ERROR_MESSAGES.PASSWORD_CHANGE_FAILED
        );
      }
    });
  }
